module ActiveMerchant #:nodoc:
  module Shipping
    ...
    class AcceptanceResponse < Response
      ...
      def save_files_for_package_with_tracking_number(tracking_number, file_path)
        package = @packages.select { |p| p[:tracking_number] == tracking_number }.first
        if package
          File.open("#{file_path}/label#{tracking_number}.gif", 'wb') do |file|
            file.write(package[:image_data])
          end
          File.open("#{file_path}/label#{tracking_number}.html", 'wb') do |file|
