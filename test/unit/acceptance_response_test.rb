...
class AcceptanceResponseTest < Test::Unit::TestCase
  ...
  def test_save_image_file
    file_path_basename_1 = '/tmp/test_save_image_file_1'
    file_path_basename_2 = '/tmp/test_save_image_file_2'

    @acceptance_response.save_files_for_package_with_tracking_number '1Z9196170297067329', file_path_basename_1
    @acceptance_response.save_files_for_package_with_tracking_number '1Z9196170295190136', file_path_basename_2

    file_contents_1_gif = open("#{file_path_basename_1}.gif", 'rb') { |io| io.read }
    file_contents_1_html = open("#{file_path_basename_1}.html", 'rb') { |io| io.read }
    file_contents_2_gif = open("#{file_path_basename_2}.gif", 'rb') { |io| io.read }
    file_contents_2_html = open("#{file_path_basename_2}.html", 'rb') { |io| io.read }
    ...
    File.delete "#{file_path_basename_1}.gif"
    File.delete "#{file_path_basename_1}.html"
    File.delete "#{file_path_basename_2}.gif"
    File.delete "#{file_path_basename_2}.html"

    error = assert_raise RuntimeError do
      @acceptance_response.save_files_for_package_with_tracking_number 'INVALID', file_path_basename_1
    end
