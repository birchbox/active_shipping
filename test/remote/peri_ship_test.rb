...
class PeriShipTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @carrier = PeriShip.new(fixtures(:peri_ship).merge(:test => true))
  end
  ...
  def test_failure_rates_request
    begin
      ...
      assert_equal 'Weight must be between 0 and 150 pounds.', e.message
    end
  end

  def test_multiple_packages_are_combined_correctly
    response_wii = @carrier.find_rates(@locations[:beverly_hills],
                                       @locations[:real_google_as_residential],
                                       @packages.values_at(:wii))
    response_book = @carrier.find_rates(@locations[:beverly_hills],
                                        @locations[:real_google_as_residential],
                                        @packages.values_at(:book))
    response_combined = @carrier.find_rates(@locations[:beverly_hills],
                                            @locations[:real_google_as_residential],
                                            @packages.values_at(:book, :wii))
